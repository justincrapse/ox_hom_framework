...
class OXNode(ParmTemplate):  # mixins
    ...
    def create_node_if_not_exists(self, ox_node_class=None, ox_node_type=None, node_name=None) -> hou.Node:
        ...
        # TODO: DELETE THIS
        # else:
        #     # if already exists, we need to clear the parms in case there are refernces
        #     for parm in child_node.parms():
    ...
    def create_ox_node(self, ox_node_class, node_name, replace=False, use_existing=False):
        if replace:
            ...
            if use_existing:
                ...
                child_node = self.create_node(node_type_name=ox_node_class.node_type, node_name=node_name)
        return ox_node_class(child_node)
    ...
    def get_child_ox_node_by_name(self, child_name, node_class=None, expect_match=True):
        ...
        if not child_node:
            ...
            child_nodes_str = child_nodes_str if child_nodes_str else "Yo, There are no child nodes at all!"
            if expect_match:
    ...
    def ox_child(self, child_name, node_class):
        ox_child = self.get_child_ox_node_by_name(child_name=child_name, node_class=node_class)
        return ox_child
    ...
    def move_node_relative_to(self, ox_node, x=0, y=-1, unit_multiplier=1):
        ...
        info_msg = f"r_x: {r_x}, r_y: {r_y}, unit_multi: {unit_multiplier}, x: {x}, y: {y}"
        ox_logger.info(info_msg)
        vector = hou.Vector2(r_x + x * unit_multiplier, r_y + y * unit_multiplier)
        self.node.setPosition(vector)

    def move_node_relative_to_other_node(self, ox_node_to_move, ox_node_to_move_relative_to, x=0, y=-1, unit_multiplier=1):
        """Move node relative to another node"""
        relative_position_vector = ox_node_to_move_relative_to.node.position()
        r_x = relative_position_vector[0]
        r_y = relative_position_vector[1]
        vector = hou.Vector2(r_x + x * unit_multiplier, r_y + y * unit_multiplier)
        ox_node_to_move.node.setPosition(vector)
    ...
    def recursive_get_child_nodes_by_type(self, node_type, ox_node_to_search=None):
        ...
        child_node_list = ox_node_to_search.get_child_nodes()
        ox_logger.info(f"Child Node List: {child_node_list}")
        matching_child_node_list = ox_node_to_search.get_child_nodes_by_type(node_type=node_type)
        if child_node_list:
            for child_node in child_node_list:
                child_ox_node = OXNode(child_node)
                matching_child_node_list.extend(self.recursive_get_child_nodes_by_type(node_type=node_type, ox_node_to_search=child_ox_node))
        return matching_child_node_list
