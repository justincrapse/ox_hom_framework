...
class HeightfieldScatterNode(HHNode):
    ...
    def __init__(self, node=None, hh_parent=None, node_name=None):
        ...
        # parm menu vars:
        self.parm_scattermethod_menu = ScattermethodMenu(parm=self.node.parm('scattermethod'))
        self.parm_perpointcount_method_menu = PerpointcountMethodMenu(parm=self.node.parm('perpointcount_method'))
        self.parm_positioning_method_menu = PositioningMethodMenu(parm=self.node.parm('positioning_method'))
        self.parm_variability_method_menu = VariabilityMethodMenu(parm=self.node.parm('variability_method'))
        self.parm_relax_pointremovalmethod_menu = RelaxPointremovalmethodMenu(parm=self.node.parm('relax_pointremovalmethod'))
        self.parm_piecemode_menu = PiecemodeMenu(parm=self.node.parm('piecemode'))
...
class ScattermethodMenu(Menu):
    def __init__(self, parm):
        ...
        self.by_coverage_using_mask_layer = 0
        self.by_density_using_mask_layer = 1
        self.total_point_count_using_mask_layer = 2
        self.per_point_count_using_source_points = 3
...
class VariabilityMethodMenu(Menu):
    def __init__(self, parm):
        self.parm = parm
        super().__init__(parm=parm)
        self.uniform_distribution = 0
        self.normal_distribution = 1
        self.exact_scale = 2
...
class PiecemodeMenu(Menu):
    def __init__(self, parm):
        self.parm = parm
        super().__init__(parm=parm)
        self.from_attribute = 0
        self.from_connectivity = 1
        self.single_piece = 2
