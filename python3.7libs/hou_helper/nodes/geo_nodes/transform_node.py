...
class TransformNode(HHNode):
    ...
    def __init__(self, node=None, hh_parent=None, node_name=None):
        ...
        self.parm_prexform_xord_menu = PrexformXordMenu(parm=self.node.parm('prexform_xOrd'))
        self.parm_prexform_rord_menu = PrexformRordMenu(parm=self.node.parm('prexform_rOrd'))
        self.parm_outputmerge_menu = OutputmergeMenu(parm=self.node.parm('outputmerge'))
...
class OutputmergeMenu(Menu):
    def __init__(self, parm):
        self.parm = parm
        super().__init__(parm=parm)
        self.replace_existing = 0
        self.pre_multiply = 1
        self.post_multiply = 2
