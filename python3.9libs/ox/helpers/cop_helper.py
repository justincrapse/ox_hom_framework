...
class COPHelper:
    ...
    def create_output_for_each_plane(self, ox_copnet_node, rename_layer_name="C"):
        ...
        for plane in planes:
            delete_node = nodes.cop_nodes.DeleteNode(
                ox_parent=self.ox_cop_node, node_name=f"{plane}_delete"
            )
            ...
            rename_node = nodes.cop_nodes.RenameNode(
                ox_parent=self.ox_cop_node, node_name=f"{plane}_rename"
            )
            rename_node.parm_from = plane
            rename_node.parm_to = rename_layer_name
            rename_node.connect_from(ox_node=delete_node)

            equalize_node = nodes.cop_nodes.EqualizeNode(
                ox_parent=self.ox_cop_node, node_name=f"{plane}_equalize"
            )
            equalize_node.connect_from(ox_node=rename_node)
            equalize_node.load_preset(plane)

            brightness_node = nodes.cop_nodes.BrightNode(
                ox_parent=self.ox_cop_node, node_name=f"{plane}_brightness"
            )
            brightness_node.connect_from(ox_node=equalize_node)
            brightness_node.load_preset(plane)

            null_node = nodes.cop_nodes.NullNode(
                ox_parent=self.ox_cop_node, node_name=f"OUT_{plane}"
            )
    ...
    def get_output_hou_nodes(self):
        out_hou_nodes = self.ox_cop_node.get_child_nodes_by_partial_name(
            substring="OUT_"
        )
