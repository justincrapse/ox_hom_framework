...
def get_hda_parms_values_by_substring(parm_substring, as_raw_value=False, null_msg=None, allow_null=True):
    hda_ox_node = OXNode(node=hou.pwd())
    parms_list = hda_ox_node.get_parms_by_name_substring(substring=parm_substring)
    parm_values_list = [
        get_hda_parm_value(parm_name=i.name(), allow_null=allow_null, as_raw_value=as_raw_value, null_msg=null_msg) for i in parms_list
    ]
    return parm_values_list


def get_hda_parms_values_by_regex(regex_str, as_raw_value=False, null_msg=None, allow_null=True):
    hda_ox_node = OXNode(node=hou.pwd())
    parameters = hda_ox_node.get_parms()
    parm_sublist = [i for i in parameters if re.match(regex_str, i.name())]
    parm_values_list = [
        get_hda_parm_value(parm_name=i.name(), allow_null=allow_null, as_raw_value=as_raw_value, null_msg=null_msg) for i in parm_sublist
    ]
    return parm_values_list
