...
def get_ox_node_from_hda_parm(
    parm_name, node_class=OXNode, null_msg=None, allow_null=True, return_hou_node=False
) -> OXNode:
    """this will return an ox node for node path parm. Just a nice shortcut"""
    ox_logger.debug(
        f'getting hda parm by name: {parm_name}. for method "get_ox_node_from_hda_parm"'
    )
    node_path_parm = hou.pwd().parm(parm_name)
    if not node_path_parm:
        raise ValueError(
            f'No parm found for parm name "{parm_name}" for hda ox node using get_ox_node_from_hda_parm method'
        )
    ...
    if not node_path and not allow_null:
        ...
        ui.display_message(
            message=null_msg, title="get_ox_node_from_hda_parm null message"
        )
...
def get_hda_parm_value(parm_name, as_raw_value=False, null_msg=None, allow_null=True):
    """
    ...
    parm_value = (
        hou.pwd().parm(parm_name).rawValue()
        if as_raw_value
        else hou.pwd().parm(parm_name).eval()
    )
...
def get_hda_parms_values_by_substring(
    parm_substring, as_raw_value=False, null_msg=None, allow_null=True
):
...
def get_hda_parms_values_by_regex(
    regex_str, as_raw_value=False, null_msg=None, allow_null=True
):
    hda_ox_node = OXNode(node=hou.pwd())
