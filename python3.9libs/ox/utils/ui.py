...
def select_one_from_list(
    options_list, message=None, title="Select One", expect_selection=True
):
    ox_logger.debug(f"select one from list options: {options_list}")
    selected = hou.ui.selectFromList(
        options_list, exclusive=True, message=message, title=title
    )
    if selected:
...
def select_many_from_list(
    options_list, message=None, title="Select One or More", expect_selection=True
):
    ox_logger.debug(f"select many from list options: {options_list}")
    selected_indexes = hou.ui.selectFromList(
        options_list, exclusive=False, message=message, title=title
    )
...
def prompt_for_string_input(message, title="Enter String", default_text=None):
    return hou.ui.readInput(
        message=message, title=title, initial_contents=default_text
    )[1]


def read_input(message, title="Enter String", default_text=None):
    return prompt_for_string_input(
        message=message, title=title, default_text=default_text
    )


def read_multi_input(
    input_labels, message, title="Enter strings", return_values_only=False
):
    string_values = hou.ui.readMultiInput(
        input_labels=input_labels, message=message, title=title
    )[1]
...
def display_confirmation(
    message, title="Confirmation Display", require_confirmation=False
):
...
def get_selected_nodes(
    ...
    expected_message="Tool Expected Node Selection",
    only_one=False,
) -> List[hou.Node]:
    ...
    if only_one and len(hou_node_list) > 1:
        display_message(
            message=f"{len(hou_node_list)} nodes selected, but only expecting one"
        )
    return hou_node_list


def get_selected_node(
    expected_message="Tool Expected Node Selection", expect_selected=True
) -> hou.Node:
    return get_single_selected_node(
        expected_message=expected_message, expect_selected=expect_selected
    )
...
def select_and_verify_single_node(
    expected_selected=True, title="Verify Selected Nodes"
):
    selected_hou_nodes = get_selected_nodes(
        expect_selected=expected_selected, only_one=True
    )
    msg = "\n".join([i.name() for i in selected_hou_nodes])
    result = display_confirmation(message=msg, title=title)
    if result:
